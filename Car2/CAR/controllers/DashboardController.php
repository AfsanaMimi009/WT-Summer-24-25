<?php
// controllers/DashboardController.php - Main dashboard controller
...
class DashboardController {
    ...
    private function validateCarData($data, $exclude_id = null) {
        ...
        } else if ($this->carModel->checkLicensePlateExists($data['license_plate'], $exclude_id)) {
    ...
    private function uploadCarImage($file) {
        ...
        if (!in_array($file['type'], $allowed_types)) {
            return array('success' => false, 'error' => 'Only JPG, PNG, and GIF images are allowed.');
        }
        
        if ($file['size'] > $max_size) {
            return array('success' => false, 'error' => 'Image size must be less than 5MB.');
        }
