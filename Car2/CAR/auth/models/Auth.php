<?php
// auth/models/Auth.php - Fixed Authentication model
...
class Auth {
    ...
    private function createSession($user, $rememberMe = false) {
        ...
        // Store session in database (only if table exists)
        try {
            ...
            // If user_sessions table doesn't exist, continue without it
        ...
        // Set remember me cookie if requested (skip if remember fields don't exist)
        if ($rememberMe) {
            try {
    ...
    private function deleteSession($sessionId) {
        try {
            ...
            // Continue if session table doesn't exist
        }
    }
    ...
    private function logLoginAttempt($identifier, $success) {
        try {
            ...
            // Continue if login_attempts table doesn't exist
        }
    }
